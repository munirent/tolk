...
module Tolk
  class Locale < ActiveRecord::Base
    ...
    def phrases_without_translation(page = nil)
      ...
      result = phrases.public_send(Kaminari.config.page_method_name, page)
    ...
    def search_phrases(query, scope, key_query, page = nil)
      ...
      phrases.public_send(Kaminari.config.page_method_name, page)
    end
...
#      phrases = phrases.scoped(:conditions => ['tolk_phrases.id NOT IN (?) AND tolk_phrases.id IN(?)', existing_ids, found_translations_ids]) if existing_ids.present?
    ...
    def find_phrases_with_translations(page, conditions = {})
      result = Tolk::Phrase.where({ :'tolk_translations.locale_id' => self.id }.merge(conditions)).joins(:translations).order('tolk_phrases.key ASC').public_send(Kaminari.config.page_method_name, page)
